...
export const HotspotAnalysisView: React.FC<HotspotAnalysisViewProps> = ({
  ...
  const isHigh = derivedRisk === 'high';

  // Dynamic contextual interventions matching the selected zone
  const displayInterventions = getContextualInterventions(selectedZone);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <h3 className="font-heading font-extrabold text-xl md:text-2xl text-slate-900 mt-1">
                {selectedZone.shortName}
              </h3>
              <p className="text-xs text-slate-600 mt-0.5">
                {isLowRisk ? (
                  <span>Primary Cooling Characteristic: <strong className="text-emerald-800">{selectedZone.primaryCause}</strong></span>
                ) : (
                  <span>Primary Thermal Driver: <strong className="text-orange-600">{selectedZone.primaryCause}</strong></span>
                )}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 bg-slate-50 p-3 rounded-xl border border-slate-200">
            ...
            <div className="px-3 border-r border-slate-200">
              ...
              <span className={`text-2xl font-mono-data font-extrabold ${
                selectedZone.diffFromSurround > 0 ? 'text-orange-600' : 'text-emerald-700'
              }`}>
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-4">
            ...
            <div className="space-y-4 pt-1">
              {selectedZone.causes.map((cause, idx) => (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                  ...
                  <span className={`text-[10px] block mt-0.5 font-bold ${
                    selectedZone.vegetation >= 50 ? 'text-emerald-700' : 'text-red-600'
                  }`}>
                    {selectedZone.vegetation >= 50 ? 'High Canopy Buffer' : 'Critical Deficit'}
                  </span>
                </div>
                ...
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                  ...
                  <span className={`text-lg font-mono-data font-extrabold ${
                    selectedZone.imperviousSurface <= 30 ? 'text-emerald-700' : 'text-orange-600'
                  }`}>
                    {selectedZone.imperviousSurface}%
                  </span>
                  <span className="text-[10px] text-slate-500 block mt-0.5">
                    {selectedZone.imperviousSurface <= 30 ? 'High Permeability' : 'High Absorption'}
                  </span>
                </div>
