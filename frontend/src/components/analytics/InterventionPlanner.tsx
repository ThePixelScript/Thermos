...
export const InterventionPlanner: React.FC<InterventionPlannerProps> = ({
  ...
  chriScore: propChriScore = 65.0,
  riskLevel: propRiskLevel = 'HIGH',
  primaryDriver: propPrimaryDriver = 'Built Surface Thermal Retention',
  surfaceTemp: propSurfaceTemp = 41.5,
  thermalAnomaly: propThermalAnomaly = 7.5,
}) => {
  const targetZoneId = detail?.zone?.id || propZoneId || 'ZONE-01';
  const targetZoneName = detail?.zone?.name || propZoneName || `Zone ${targetZoneId}`;
  const targetChri = detail?.risk_assessment?.risk_score?.score ?? propChriScore;
  const targetRiskLevel = detail?.risk_assessment?.risk_score?.risk_level || propRiskLevel;
  const targetPrimaryDriver =
    detail?.risk_assessment?.risk_score?.driver_contributions?.[0]?.name ||
    detail?.summary?.dominant_driver ||
    propPrimaryDriver;
  const targetSurfaceTemp = detail?.zone?.thermal_observation?.land_surface_temp_c ?? propSurfaceTemp;
  const targetAnomaly = detail?.zone?.thermal_observation?.thermal_anomaly_c ?? propThermalAnomaly;
  ...
  }, [targetZoneId, targetZoneName, selectedIds, budgetLakhs]);
    ...
    <div className="planner-container">
      ...
      <div className="planner-zone-banner">
        <div className="planner-zone-left">
          ...
          <h3 className="planner-zone-name">{targetZoneName}</h3>
          <div className="planner-zone-chips">
            <span className="chip zone-chip">{targetZoneId}</span>
            <span className={`chip risk-chip ${targetRiskLevel.toLowerCase()}`}>
              CHRI: {targetChri.toFixed(1)} ({targetRiskLevel})
            </span>
            <span className="chip driver-chip">Primary Driver: {targetPrimaryDriver}</span>
          </div>
        </div>
        <div className="planner-zone-temp">
          <span className="temp-val">{targetSurfaceTemp.toFixed(1)}°C</span>
          <span className="temp-sub">
            {targetAnomaly > 0 ? '+' : ''}
            {targetAnomaly.toFixed(1)}°C vs Baseline
          </span>
        </div>
      </div>
        ...
        <div className="catalog-header">
          <div>
            ...
            <p className="catalog-sub">
              Select evidence-based measures calibrated to {targetZoneName} morphology
            </p>
          </div>
            ...
            <div className="modal-header">
              <div>
                ...
                <p className="modal-sub">
                  Target: <strong>{targetZoneName} ({targetZoneId})</strong> • CHRI: {targetChri.toFixed(1)} ({targetRiskLevel})
                </p>
              </div>
              <button
                type="button"
